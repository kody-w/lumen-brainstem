...
> **Frozen excerpt** of the canonical rappid contract (`rapp-rappid/2.0`). Bundled at planting time on 2026-05-09T15:45:28Z. Updated to the consolidated Eternity rappid format (Constitution Art. XXXIV.1, locked 2026-06-03).
...
No `v2:`/`v3:` prefix, no inline `<kind>` segment, no `@github.com/...` suffix — `@<owner>/<slug>` is self-locating (`github.com/<owner>/<slug>` is the door). `kind` now lives in the `rappid.json` record (a field), not the string.

Example (this neighborhood's):

```
rappid:@kody-w/lumen-brainstem:<hex>
```
...
| `@<owner>/<slug>` | The GitHub composite identity / location. The `@` prefix is literal and required. `github.com/<owner>/<slug>` is the door. |
| `<hex>` | The identity hash, lowercase hex. Minted ONCE at planting; permanent thereafter (existing 32-hex hashes are grandfathered — never regenerated). |

> Legacy forms (`rappid:v2:<kind>:@<owner>/<repo>:<32hex>@github.com/<owner>/<repo>`, bare UUIDs) are read forever and canonicalized into the form above via `tools/door_address.py::canonicalize_rappid`; only the consolidated form is emitted.
